...
import (
	...
	UserModel "personal-finance-api/models/user"
	"strconv"
...
func GetById(fiberCtx *fiber.Ctx) error {
	...
	result, err := UserModel.GetById(ctx, id)
	if err != nil {
...
func Create(fiberCtx *fiber.Ctx) error {
	...
	result, err := UserModel.Create(ctx, payload)
	if err != nil {
		return fiber.NewError(fiber.StatusBadRequest, err.Error())
	}
...
func Update(fiberCtx *fiber.Ctx) error {
	...
	id, _ := strconv.Atoi(fiberCtx.Params("id"))
	err := UserModel.Update(ctx, id, payload)
	if err != nil {
		return fiber.NewError(fiber.StatusBadRequest, err.Error())
	}
